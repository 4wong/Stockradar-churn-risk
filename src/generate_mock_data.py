...
def generate_latents() -> MerchantLatents:
    return MerchantLatents(
        tech_risk=float(np.random.beta(2.2, 7.2)),
        engagement=float(np.random.beta(2.8, 3.2)),
        roi_strength=float(np.random.beta(2.0, 5.2)),
    )


def churn_probability(lat: MerchantLatents, roi_ratio: float, fail_rate: float, integ_30d: float) -> float:
    roi_term = -1.7 * (roi_ratio - 1.0)
    fail_term = 3.6 * (fail_rate - 0.10)
    integ_term = 1.8 * (integ_30d - 0.06)
    tech_term = 0.9 * (lat.tech_risk - 0.25)
    engage_term = -0.8 * (lat.engagement - 0.5)

    logit = 0.55 + roi_term + fail_term + integ_term + tech_term + engage_term
    return float(clamp(sigmoid(logit), 0.03, 0.95))
...
def generate_app_events(
    ...
    base_dash = 0.18 + 0.95 * lat.engagement
    base_sms = 0.16 + 0.55 * lat.engagement
    base_err = 0.02 + 0.30 * lat.tech_risk
    ...
    for day_offset in range((end_dt - install_dt).days):
        ...
        if random.random() < base_dash * mult:
            n = 1 + (1 if random.random() < 0.25 else 0)
            for _ in range(n):
        ...
        if random.random() < base_sms * mult:
            attempts = 1 + (1 if random.random() < 0.35 else 0)
            for _ in range(attempts):
                sms_sent += 1
                fail_p = clamp(0.08 + 0.70 * lat.tech_risk, 0.05, 0.75)
    ...
    stats = {
        "fail_rate": float(sms_failed / sms_sent) if sms_sent > 0 else 0.0,
        "sms_sent": float(sms_sent),
        "integration_error_rate": float(integ_err / max((end_dt - install_dt).days, 1)),
    }
    return events, stats
...
def generate_sales(m_id: int, install_dt: date, end_dt: date, lat: MerchantLatents) -> List[Tuple[int, str, float]]:
    ...
    for day_offset in range((end_dt - install_dt).days):
        ...
        if random.random() < 0.07:
            continue
        base = 5.2 * (0.35 + 1.15 * lat.engagement) * (0.25 + 1.05 * lat.roi_strength) * seasonal_multiplier(d)
        val = clamp(base * float(np.random.lognormal(0, 0.7)), 0.0, 650.0)
        if random.random() < (0.30 - 0.18 * lat.engagement):
            val *= 0.05
...
def main() -> None:
    ...
    for m_id in range(1, N_MERCHANTS + 1):
        ...
        if adopts_pro:
            ...
            churns = random.random() < churn_probability(lat, float(roi), float(stats["fail_rate"]), float(stats["integration_error_rate"]))
